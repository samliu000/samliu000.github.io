...
function showDialog(method, blogIndex = -1) {
  if (method == "add") {
    blogDialog.innerHTML = `<form method="dialog">
        ...
        <input type="text" id="blog-title-input"></input><br>

        <label for="blog-summary-input">Summary</label><br>
        <textarea id="blog-summary-input" rows="5" cols="70"></textarea><br>

        <button id="submit-blog-post">Submit</button>
    </form>`;
  } else if (method == "edit") {
    const blogPost = blogPosts[blogIndex];

    blogDialog.innerHTML = `<form method="dialog">
        <legend>Edit Blog Post</legend>
        <label for="blog-title-input">Title</label><br>
        <input type="text" id="blog-title-input" value="${blogPost.title}"></input><br>

        <label for="blog-summary-input">Summary</label><br>
        <textarea id="blog-summary-input" rows="5" cols="70">${blogPost.summary}</textarea><br>
